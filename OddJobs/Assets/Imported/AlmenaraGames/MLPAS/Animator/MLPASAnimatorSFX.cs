...
namespace AlmenaraGames
{
    ...
    public class MLPASAnimatorSFX : StateMachineBehaviour
    {
        ...
        public class StateSFX
        {
            ...
            public bool cOverridePlayTime = false;
            public bool cOverrideCustomParameters = false;
            public bool cOverrideAudioObject = false;
            public bool cOverrideChannel = false;
            public bool cOverrideListenerPause = false;

            public float cPlayTime = -1f;
            public bool cUseIdentifier = false;
            public string cIdentifier = string.Empty;
            public AudioObject cAudioObject = null;
            public bool cUseChannel = false;
            public int cChannel = -1;
            public bool cIgnoreListenerPause = false;

            public bool cBoolParameter;
            public float cFloatParameter;
            public int cIntParameter;
            public string cStringParameter;
            public UnityEngine.Object cObjectParameter;
            ...
            public class OverrideValues
            {
                ...
                public bool OverridePlayTime;
                public bool OverrideCustomParameters;
                public bool OverrideAudioObject;
                public bool OverrideChannel;
                public bool OverrideListenerPause;

                public float playTime;
                public MLPASACustomPlayMethodParameters.CustomParams customParams;
                public bool useIdentifier;
        ...
        public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            for (int i = 0; i < stateSfxs.Count; i++)
            {
               ...
               int blendValue = GetBlendValue(stateSFX, animator);
                ...
                if (useExternalController && sfxValues.HasStateValue(stateSFX.ID) && sfxValues.GetStateValue(stateSFX.ID).OverridePlayTime)
                {
                    stateSfxs[i].normalizedPlayTime = sfxValues.GetStateValue(stateSFX.ID).playTime / 100;
                }
            ...
            for (int i = 0; i < stateSfxs.Count; i++)
            {
                ...
                if (stateSFX.playMode == MLPASAnimatorPlayMode.OnUpdate && !onStateMachine)
                {
                   ...
                   if (stateSFX.lastNormalizedTime > stateInfo.normalizedTime)
                    ...
                    bool _playOnlyOnce = !stateInfo.loop || stateSFX.playOnlyOnce;
                    ...
                    bool playOnceTest = _playOnlyOnce && !stateSFX.sfxPlayed && (stateSFX.curLoop == stateSFX.playCycle || stateSFX.playCycle == -1 || !stateInfo.loop);
        ...
        void PlaySFX(AnimatorStateInfo stateInfo, StateSFX stateSfx, int blendValue, Animator anim)
        {
            ...
            bool _containsOverrideValues = useExternalController && sfxValues.HasStateValue(stateSfx.ID);

            MLPASACustomPlayMethodParameters.CustomParams _customParams = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideCustomParameters ? sfxValues.GetStateValue(stateSfx.ID).customParams : stateSfx.userParameters;

            bool _useIdentifier = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideAudioObject ? sfxValues.GetStateValue(stateSfx.ID).useIdentifier : stateSfx.useIdentifier;
            string _aoIdentifier = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideAudioObject ? sfxValues.GetStateValue(stateSfx.ID).identifier : stateSfx.audioObjectIdentifier;
            AudioObject _audioObject = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideAudioObject ? sfxValues.GetStateValue(stateSfx.ID).audioObject : stateSfx.audioObject;

            bool _ignoreListenerPause = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideListenerPause ? sfxValues.GetStateValue(stateSfx.ID).ignoreListenerPause : stateSfx.IgnoreListenerPause;

            bool _useChannel = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideChannel ? sfxValues.GetStateValue(stateSfx.ID).useChannel : stateSfx.useChannel;
            int _channel = _containsOverrideValues && sfxValues.GetStateValue(stateSfx.ID).OverrideChannel ? sfxValues.GetStateValue(stateSfx.ID).channel : stateSfx.channel;
            ...
            if (!stateSfx.useCustomPlayMethod && !isBlendTree)
            {
                if (_useIdentifier)
                {
                    if (Object.ReferenceEquals(MultiAudioManager.GetAudioObjectByIdentifier(_aoIdentifier, false), null))
                        Debug.LogWarning(stateSfx.pathName + " | <i>Audio Object</i> is missing or invalid");
                }
                else
                {
                    if (Object.ReferenceEquals(_audioObject, null))
                        Debug.LogWarning(stateSfx.pathName + " | <i>Audio Object</i> is missing or invalid");
                }
            }
#endif
            ...
            if (!isBlendTree)
            {
                ...
                if (stateSfx.useCustomPlayMethod)
                {
                    if (stateSfx.customPlayMethod != null)
                    {
                        if ((stateSfx.customPlayMethod.Target as Object) != null)
                        {
                            ...
                            MLPASACustomPlayMethodParameters.CustomParams userParameters = _customParams;
                            MLPASACustomPlayMethodParameters parameters = new MLPASACustomPlayMethodParameters(_useIdentifier ? MultiAudioManager.GetAudioObjectByIdentifier(_aoIdentifier) : _audioObject, _useChannel ? _channel : -1, followPos, trans.position, trans, 0, stateInfo, stateSfx.normalizedPlayTime, userParameters);
                ...
                if (followPos)
                {
                    ...
                    if (_useIdentifier)
                    {
                        MultiAudioSource _source = MultiAudioManager.PlayAudioObjectByIdentifier(_aoIdentifier, _useChannel ? _channel : -1, usingDifferentPlayPos ? sfxValues.playPosition : trf);
                        _source.IgnoreListenerPause = _ignoreListenerPause;
                    }
                    else
                    {
                        MultiAudioSource _source = MultiAudioManager.PlayAudioObject(_audioObject, _useChannel ? _channel : -1, usingDifferentPlayPos ? sfxValues.playPosition : trf);
                        _source.IgnoreListenerPause = _ignoreListenerPause;
                    }
                ...
                else
                {
                    ...
                    if (_useIdentifier)
                    {
                        MultiAudioSource _source = MultiAudioManager.PlayAudioObjectByIdentifier(_aoIdentifier, _useChannel ? _channel : -1, usingDifferentPlayPos ? sfxValues.playPosition.position : trf.position);
                        _source.IgnoreListenerPause = _ignoreListenerPause;
                    }
                    else
                    {
                        MultiAudioSource _source = MultiAudioManager.PlayAudioObject(_audioObject, _useChannel ? _channel : -1, usingDifferentPlayPos ? sfxValues.playPosition.position : trf.position);
                        _source.IgnoreListenerPause = _ignoreListenerPause;
                    }
        ...
        public bool UpdateRuntimeValues()
        {
            ...
            for (int i = 0; i < stateSfxs.Count; i++)
            {
                ...
                if (!stateSfxs[i].IDFinded)
                {
                    stateSfxs[i].IDFinded = true;

                    stateSfxs[i].ID = globalAddedSfx - i;
        ...
        public class MLPASAnimatorSFXEditor : UnityEditor.Editor
        {
            ...
            UnityEditor.Editor motionEditorPreview;
            ...
            public override void OnInspectorGUI()
            {
                ...
                if ((obj.targetObject as MLPASAnimatorSFX).hasID)
                {

                    EditorGUILayout.Space();

                    EditorGUILayout.LabelField("State Behaviour ID: " + (obj.targetObject as MLPASAnimatorSFX).id, EditorStyles.miniLabel);
                ...
                if (!isPlaying)
                {
                    ...
                    if (GUILayout.Button(pasteIcon, smallButton))
                    {
                        ...
                        if (config != null)
                        {
                            ...
                            newSFX.IDFinded = true;
                            newSFX.ID = (obj.targetObject as MLPASAnimatorSFX).globalAddedSfx;
                    ...
                    if (stateSfxsProp.Count > 0)
                    {
                        ...
                        for (int i = 0; i < stateSfxsProp.Count; i++)
                        {
                            ...
                            if (!stateSfxsProp[i].IDFinded)
                            {
                                stateSfxsProp[i].IDFinded = true;

                                stateSfxsProp[i].ID = (obj.targetObject as MLPASAnimatorSFX).globalAddedSfx - i;
                            }

                            if (GUILayout.Button(" <b>" + i.ToString("D2") + "-</b>" + (stateSfxsProp[i].ignoreSfx ? "<color=red><b>IGNORE</b></color> | " : "") + aoName + " <b>[" + stateSfxsProp[i].playMode.ToString() + "]</b>" + (extraName), itemStyle))
                            {
                                selectedIndex = i;
                    ...
                    if (stateSfxsProp.Count > 0 && selectedIndex >= 0 && (obj.targetObject as MLPASAnimatorSFX).stateSfxs.Count > 0 && selectedIndex < (obj.targetObject as MLPASAnimatorSFX).stateSfxs.Count)
                    {
                        ...
                        BoolField("Ignore Listener Pause", "Plays ignoring the listener pause", ref (obj.targetObject as MLPASAnimatorSFX).stateSfxs[selectedIndex].IgnoreListenerPause);
                        ...
                        BoolField("Follow Animator Position", "Follows the Animator position when played", ref (obj.targetObject as MLPASAnimatorSFX).stateSfxs[selectedIndex].followAnimator);
                ...
                else
                {
                    ...
                    if (stateSfxsProp.Count > 0)
                    {
                        ...
                        for (int i = 0; i < stateSfxsProp.Count; i++)
                        {
                            ...
                            if (GUILayout.Button(" <b>" + i.ToString("D2") + "-</b>" + aoName + " <b>[" + stateSfxsProp[i].playMode.ToString() + "]</b>" + (extraName), itemStyle))
                            {
                                selectedIndex = i;
                       ...
                       /* EditorGUILayout.Space();
                        ...
                        BoolField("Ignore Listener Pause", (obj.targetObject as MLPASAnimatorSFX).stateSfxs[selectedIndex].IgnoreListenerPause);
                ...
                if (!(obj.targetObject as MLPASAnimatorSFX).hasID)
                {
                    (obj.targetObject as MLPASAnimatorSFX).id = (obj.targetObject as MLPASAnimatorSFX).GetInstanceID();
                    (obj.targetObject as MLPASAnimatorSFX).hasID = true;
                    valuesModified = true;
                }
