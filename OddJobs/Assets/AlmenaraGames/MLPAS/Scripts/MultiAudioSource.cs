...
namespace AlmenaraGames{
	...
	[HideInInspector] public float NormalizedLength = 1f;
	...
	private float pitch;
	private float prevLoopPitchMultiplier;
	private float dopplerLevel;
	private bool ignoreGlobalDopplerFactor;
	...
	private bool onStopInvoked = false;
	...
	public bool Loop{get{return loop;}}
		...
		/// <summary>
		/// Gets the current playing clip of this <see cref="MultiAudioSource"/>.
		/// </summary>
		public AudioClip CurrentClip { get { return audioSource.clip; } }

		/// <summary>
		/// Gets the current pitch of the clip playing at this <see cref="MultiAudioSource"/>.
		/// </summary>
		public float CurrentPitch { get { return pitch; } }


		public bool OverridePitch { get { return overridePitch; } }
	...
	private bool insidePool = false;
       ...
       AnimationCurve logCurve;
        ...
        public UnityEngine.Events.UnityEvent OnPlay = new UnityEngine.Events.UnityEvent();
        public UnityEngine.Events.UnityEvent OnStop = new UnityEngine.Events.UnityEvent();
        public UnityEngine.Events.UnityEvent OnRange = new UnityEngine.Events.UnityEvent();
        public UnityEngine.Events.UnityEvent OnOutOfRange = new UnityEngine.Events.UnityEvent();
        public UnityEngine.Events.UnityEvent OnLoop = new UnityEngine.Events.UnityEvent();
	...
	public void DisableOverridePitch()
    {
			overridePitch = false;
    }
	...
	void Update () {
			...
			{
				bool _checkAudioObjectNULL = Object.ReferenceEquals(audioObject, null);

				if (_checkAudioObjectNULL)
				{
			...
			if (onFocus && (!MultiAudioManager.Paused || IgnoreListenerPause) && !globalPaused && !play && playing && (true && !loop && (!audioSource.isPlaying || audioSource.time >= (audioSource.clip.length * NormalizedLength) - 0.01f) || pitch<0 && loop && audioSource.time <= 0.01f || pitch>=0 && loop && audioSource.time >= (audioSource.clip.length * NormalizedLength) - 0.01f || true && loop && !audioSource.isPlaying)) {
				...
				if (loop) {
					...
					doingLoop = true;
					prevLoopPitchMultiplier = pitchRandomStartMultiplier;
			...
			if (play || !InsidePool && playOnStart) {
              //  if (!MultiAudioManager.Paused || MultiAudioManager.Paused && IgnoreListenerPause)
				delay = Mathf.Clamp (delay - (DelayUpdateMode == MultiAudioManager.UpdateModes.UnscaledTime || IgnoreListenerPause ? customUnscaledDeltaTime:Time.deltaTime), 0, Mathf.Infinity);
			...
			if (stopDelay && playing) {
				...
				stopDelayTime = stopDelayTime - (DelayUpdateMode == MultiAudioManager.UpdateModes.UnscaledTime || IgnoreListenerPause ? customUnscaledDeltaTime:Time.deltaTime);
	...
	void LatePlay()
	{
			...
			if (!nearestListenerNULL && nearestListener.ReverbPreset != AudioReverbPreset.Off) {
				waitForReverb = (audioSource.clip.length * NormalizedLength) + listenerReverb.decayTime + 0.25f;
				audioSource.bypassReverbZones = false;
			} else {
           ...
           if (dopplerLevel > 0 && (play || playing))
            {
                ...
                DopplerFactor *= Mathf.Clamp(ignoreGlobalDopplerFactor ? 1f : MultiPoolAudioSystem.audioManager.config.dopplerFactor, 0.0001f, 10);

                Vector3 dopplerCenter = /*usingShape ? shape.bounds.center : */thisPosition;

                Vector3 sourceSpeed = (lastSourcePosition - dopplerCenter) / customUnscaledDeltaTime;
                lastSourcePosition = dopplerCenter;

                Vector3 listenerSpeed = (lastListenerPosition - nearestListenerPosition) / customUnscaledDeltaTime;
                lastListenerPosition = nearestListenerPosition;
    ...
    void LateUpdate()
	{
			...
			if (OverrideValues && overrideVolumeRolloff || (!OverrideValues || !overrideVolumeRolloff) && (playing || play) && audioObject.volumeRolloff) {
				...
				if (outOfRange || noListener) {
					...
					if (channel == 3)
						Debug.Log("ESTE ES ER MARDITO BUG:");
	...
	void SetParameters(bool onUpdate=false)
	{
			if (!onUpdate)
			{
				...
				if (doingLoop)
				{
					pitchRandomStartMultiplier = prevLoopPitchMultiplier;
					doingLoop = false;
				}
		...
		if (!OverrideValues || !overridePitch) {
			...
			ignoreGlobalDopplerFactor = false;//audioObject.ignoreGlobalDopplerFactor;
    ...
    void RealPlay(float _delay = 0, int _channel = -1, Transform _targetToFollow = null, float _fadeInTime = 0f, bool looping = false)
	{
		...
		AudioClip _clip = null;
			...
			if (audioObject.avoidPlayingSameClipTwice)
			{
				_clip = audioObject.RandomClipWithIndex(out _currentClipIndex);
			}
			else
			{
				_clip = audioObject.RandomClip;
			}

			bool _clipNULL = Object.ReferenceEquals(_clip, null);
			int _clipsLength = audioObject.clips.Length;
			...
			if (audioObject.avoidPlayingSameClipTwice)
			{
				if (_clipsLength > 0)
				{
					if (!insidePool)
					{
						...
						if (_currentClipIndex==lastClipIndex)
						{


							lastClipIndex = _currentClipIndex+1;

					

							if (lastClipIndex >= _clipsLength)
							{
							...
							_clip = audioObject.clips[lastClipIndex];

						}
						else
						{
							lastClipIndex = _currentClipIndex;
						}
					...
					{
						int _tempIndex = audioObject.index;

						if (_currentClipIndex == _tempIndex)
						{


							_tempIndex = _currentClipIndex + 1;



							if (_tempIndex >= _clipsLength)
							{
							...
							_clip = audioObject.clips[_tempIndex];

						}
						else
						{
							_tempIndex = _currentClipIndex;
						}

						audioObject.index = _tempIndex;
			...
			{
				if (!_clipNULL && audioObject.loop && audioObject.playClipsSequentially)
				{

					if (loopIndex >= _clipsLength)
						loopIndex = 0;
...
/*
	...
	public void Stop(bool disableObject=false)
	{
		...
		/// <param name="disableObject">If set to <c>true</c> disable object.</param>
		public void FadeOutEvent(float _fadeOutTime = 1f)
		{
	...
	public class MultiAudioSourceEditor: UnityEditor.Editor 
	{
